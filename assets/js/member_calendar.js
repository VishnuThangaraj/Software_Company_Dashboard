...
const update_counters = (month, year) => {
  fetch(`${localhost}/api/get_event_month_mbr`, {
    method: "POST",
    body: JSON.stringify({
      month: month,
      year: year,
  ...
  for (let i = 0; i < 6 * 7; i++) {
    ...
    if (i >= startDay && i - startDay < daysInMonth) {
      ...
      cell.addEventListener("click", () => {
        ...
        fetch(`${localhost}/api/get_events_mbr`, {
          method: "POST",
          body: JSON.stringify({
...
async function fetchEventsCount(date) {
  const response = await fetch(`${localhost}/api/get_events_count_mbr`, {
    method: "POST",
    body: JSON.stringify({ date, member: developer_id }),
    headers: {
      "Content-type": "application/json; charset=UTF-8",
    },
  });
