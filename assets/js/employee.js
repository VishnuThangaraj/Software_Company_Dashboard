...
const teamlead_employee = document.getElementById(`teamlead_employee`);
...
const make_employee_table_ready = () => {
    ...
    .then((data) => {
      ...
      if (data.length > 0) {
        data.forEach((employee) => {
          ...
          option1.text = employee.name;
          option1.value = employee.id;
          edit_employee_teamlead_list.append(option1);
          ...
          temp += `<td>
                    <div class="btn btn-secondary py-1 my-2 me-1 px-3" onclick="make_edit_employee_visible(event, 'teamlead')">
                      <i class="far fa-edit"></i>
                    </div>
                    <div class="btn btn-danger  py-1 my-2 px-3" onclick="delete_employee(event,'teamlead')">
                      <i class="fas fa-trash-alt"></i>
                    </div>
                  </td>`;
          temp += `</tr>`;
          employee_table.innerHTML = temp;
          teamlead_employee.innerHTML = data.length;
        });
      }
    });
  // Fetch Employee
  ...
  fetch(`${localhost}/api/get_bench_employee_count`)
    .then((response) => response.json())
    .then((data) => {
...
const save_new_employee = () => {
  ...
  if (employee_position_list.value == "Team Lead") {
    fetch(`${localhost}/api/add_teamlead`, {
      method: "POST",
      body: JSON.stringify({
      ...
      .then((json) => {
        let content = {
          message: "New Teamlead Added to the Database",
          title: "Teamlead Added",
          icon: "icon-user",
          url: "employee.html",
          target: "_blank",
        };
    ...
    fetch(`${localhost}/api/add_employee`, {
      ...
      body: JSON.stringify({
        ...
        address: new_employee_address.value,
        designation: new_employee_designation.value,
        team_lead_id: employee_teamlead_list.value,
      }),
      headers: {
        "Content-type": "application/json; charset=UTF-8",
      },
...
const open_team_lead_list = () => {
  if (employee_position_list.value == "Developer") {
    employee_teamlead_list.disabled = false;
  } else {
    employee_teamlead_list.disabled = true;
  }
};

// DISPLAY TEAM LEAD LIST FOR EDIT
const open_team_lead_list_edit = () => {
  if (edit_employee_position_list.value == "Developer") {
    edit_employee_teamlead_list.disabled = false;
  } else {
    edit_employee_teamlead_list.disabled = true;
  }
};
...
const make_edit_employee_visible = (event, position) => {
  ...
  fetch(`${localhost}/api/get_${current_edit_position}_with_id`, {
    ...
    .then((json) => {
      ...
      if (current_edit_position == "teamlead") {
        edit_employee_position_list.value = `Team Lead`;
        edit_employee_teamlead_list.disabled = true;
      } else {
        edit_employee_position_list.value = `Developer`;
        edit_employee_teamlead_list.disabled = false;
        edit_employee_teamlead_list.value = json[0].team_lead_id;
      }
    });
};
...
const save_edited_employee = () => {
  ...
  if (current_edit_position == "teamlead") {
    if (edit_employee_position_list.value == `Team Lead`) {
      // UPDATE TEAMLEAD DETAILS
      fetch(`${localhost}/api/update_teamlead_with_id`, {
        method: "PUT",
        body: JSON.stringify({
          id: current_edit_employee,
      ...
      // Remove TeamLead and Set as Employee
      fetch(`${localhost}/api/delete_teamlead`, {
        method: "DELETE",
        body: JSON.stringify({
          id: current_edit_employee,
        }),
          ...
          if (json.success) {
            ...
            fetch(`${localhost}/api/add_employee`, {
              ...
              body: JSON.stringify({
                ...
                address: edit_employee_address.value,
                designation: edit_employee_designation.value,
                team_lead_id: edit_employee_teamlead_list.value,
              }),
              headers: {
                "Content-type": "application/json; charset=UTF-8",
              },
    ...
    if ((edit_employee_position_list.value = `Developer`)) {
      ...
      // Remove TeamLead and Set as Employee
      fetch(`${localhost}/api/delete_employee`, {
        method: "DELETE",
        body: JSON.stringify({
          id: current_edit_employee,
        }),
